...
exports.getQuestions = (req, res) => {
  return authentication.validateUser(req).then((userExist) => {
    return model.question.findAll({ raw: true, attributes: { exclude: ['answer', 'createdAt', 'updatedAt', 'section_id'] }, where: { section_id: 1 }, order: [ Sequelize.fn('RANDOM') ], limit: 20}).then((verbal) => {
      ...
      return model.question.findAll({ raw: true, attributes: { exclude: ['answer', 'createdAt', 'updatedAt', 'section_id'] }, where: { section_id: 3 }, order: [Sequelize.fn('RANDOM')], limit: 20 }).then((quantitative) => {
        ...
        return model.question.findAll({ raw: true, attributes: { exclude: ['answer', 'createdAt', 'updatedAt', 'section_id'] }, where: { section_id: 2 }, order: [Sequelize.fn('RANDOM')], limit: 20 }).then((logical) => {
